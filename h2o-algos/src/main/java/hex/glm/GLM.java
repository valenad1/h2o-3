...
/*            LineSearchSolver ls = (_state.l1pen() == 0)
    ...
    private double[] fitCoeffs(int[] randCatLevels, int totRandCatLevels, Frame returnFrame, int[] dinfoWCol, int[] wCol, 
      ...
      // calculate weight for augmented part of augXZ.  It is the lower rows 0 | Iq*wpsi
    ...
    public double[] calculate_all_beta(double[] start_delta, Frame augXZ, Frame augZW, int totRandCatLevels, 
      ...
      cleanupHGLMMemory(new DataInfo[]{qTinfo, augXZInfo}, new Frame[]{qTransposed, qTAugZW}, null, null);
    ...
    private void fitHGLM() {
      ...
      Frame augXZ = makeZeroOrOneFrame(_dinfo._adaptedFrame.numRows() + ArrayUtils.sum(_randC),
              _state.beta().length + _state.ubeta().length, 0, null);
      CalculateAugXZ calculateAugXZ = new CalculateAugXZ(_job, _dinfo, _parms, augXZ, _randC,
...
//        });
    ...
    private void scoreAndUpdateModelHGLM(GLMModel fixedModel, GLMModel[] randModels, Frame glmmmeReturns, 
                                         ...
                                         double convergence, double[][] cholR, Frame augXZ) {
      ...
      if (_parms._calc_like) { // computation/memory intensive, only calculated it if needed
    ...
    private void calHlikStuff(ModelMetricsHGLM.MetricBuilderHGLM mmHGLMBd, Frame glmmmeReturns, Frame augXZ) {
      new CalculateAugXZ(_job, _dinfo, _parms, augXZ, _randC, _state.beta(), _state.ubeta(),
              _state.get_psi(), _state.get_phi(), _state.get_tau(),
              _state.get_correction_HL()).doAll(_dinfo._adaptedFrame);  // update wdata calculation
      new CalculateAugXZRand(_job, _parms, augXZ.numCols(), augXZ.numRows(), _randC, _state.get_psi(),
              _state.get_phi(), _state.get_priorw_wpsi(), _state.get_tau(), _state.ubeta()).doAll(augXZ);// weight for random columns
